...
for ax, (sensor_board_name, df_sensor_dict) in zip(axes, df_sensors_dict.items()):
    for sensor_key in sensor_keys:
        ax.plot(
            pd.to_datetime(df_sensor_dict[timestamp_col]).values,
            df_sensor_dict[sensor_key].values,
            label=sensor_key,
        )
    ax.grid()
