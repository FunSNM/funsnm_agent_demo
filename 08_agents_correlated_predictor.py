## Implement Correlated Predictor Agent
...
def custom_plot_function(data, sender_agent, cov_factor=2, sensor_keys=["O3", "PM10", "PM25"]):
    ...
    for i, measurand in enumerate(sensor_keys):
        # Retrieve data
        prediction = np.array(data[measurand + " - Prediction"])
        ...
        lower = prediction - cov_factor * uncertainty
        upper = prediction + cov_factor * uncertainty
...
class CorrelatedPredictorAgent(MetrologicalAgent):
    def init_parameters(self, aggregate_keys=["O3", "PM10", "PM25"], max_seconds=2, metadata={}):
        super(CorrelatedPredictorAgent, self).init_parameters(self)
        ...
        if len(buffer_values) == n_input_agents:
            ...
            if np.max(time_diffs) < self.max_seconds:
                ...
                for key in self.aggregate_keys:
                    ...
                    # Calculate covariance matrix of the sensor data and covariance with predicted value
                    cov_mat = self.get_sensor_covariance(sensor_data_no_nan, sensor_names)
                    prediction_coords = self.metadata['prediction']["loc"]
                    prediction_timesteps = self.metadata['prediction']["nr_time_steps"]
                    cov_with_predicted_point = self.get_covariance_with_prediction(
                        prediction_coords, prediction_timesteps, sensor_data_no_nan, sensor_names)

                    # Calculate uncertainty weighted mean and its uncertainty
                    sensor_data_flat = np.concatenate(sensor_data_no_nan)
                    pred_val, pred_unc = self.get_prediction(sensor_data_flat, cov_mat, cov_with_predicted_point)
                    output_dict.update({key + " - Prediction": pred_val,
                                        key + " - Uncertainty": pred_unc,
    ...
    def get_covariance_with_prediction(self, prediction_coords, prediction_time_steps, sensor_data, sensor_names):
        nr_data_points = sum([len(sensor_data_i) for sensor_data_i in sensor_data])
        dist_mat = np.zeros(nr_data_points + 1)
        idx_dist = 0
        for idx_sensor in range(len(sensor_names)):
            for idx_data in range(len(sensor_data[idx_sensor])):
                dist_mat[idx_dist] = self.get_sensor_distance(
                    prediction_coords, self.metadata[sensor_names[idx_sensor]]["loc"])
                idx_dist += 1
        dist_mat[-1] = 0

        time_data = np.zeros(nr_data_points + 1)
        idx_time = 0
        for i, sensor_name in enumerate(sensor_names):
            idx_time_new = idx_time + len(sensor_data[i])
            time_per_meas = self.metadata[sensor_name]["time_dif"].astype('timedelta64[m]') / 60  # time dif in hours
            time_data[idx_time:idx_time_new] = np.arange(len(sensor_data[i])) * time_per_meas
            idx_time = idx_time_new
        time_dif_mat = time_data - (np.max(time_data) + prediction_time_steps) * time_per_meas.astype("float")
        time_dif_mat[-1] = 0

        # Determine covariance based on distance and time differences
        cov_mat_with_prediction = self.kernel(dist_mat, time_dif_mat)
        return cov_mat_with_prediction

    def get_prediction(self, x, cov, cov_with_prediction):
        # Instantiate objects
        x = np.asarray(x)
        cov = np.asarray(cov)
        v = np.asarray(cov_with_prediction)[:-1]

        # Solve C⁻¹x and C⁻¹v without explicitly inverting C
        c_inv_x = solve(cov, x, assume_a='pos')
        c_inv_v = solve(cov, v, assume_a='pos')

        # Determine predicted value and its uncertainty
        pred_val = np.dot(v, c_inv_x)
        pred_unc = np.sqrt(cov_with_prediction[-1] - np.dot(v, c_inv_v))
...
def main():
    ...
    if demo_name == "helsinki":
        ...
        mean_loc = np.mean([metadata[key]["loc"] for key in metadata.keys()], axis=0)
        metadata["prediction"] = {"loc": mean_loc, # Location of predicted value
                                  "nr_time_steps": 24} # Number of timesteps prediction is after last observation
    ...
    aggregator_agent = agentNetwork.add_agent(
        agentType=CorrelatedPredictorAgent, aggregate_keys=sensor_keys, buffer_size=window_size
    )
    ...
    agg_monitor_agent = agentNetwork.add_agent(agentType=MonitorAgent,
                                               name="Predicted value (based on last {:d} datapoints)".format(window_size))
